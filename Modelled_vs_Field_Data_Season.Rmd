...
timeseries <- timeseries %>%
  ...
  mutate(springyear = as.integer(format(time_cnrm,"%Y"))) %>%
  select(codigo, cli_estacion_id, time_cnrm, springyear, tas_cnrm, tas_ichec, tas_ipsl, tas_mohc, tas_mpi) %>%
...
```{r}
spring <- read.csv(paste(path, folder, "spring_mean.csv", sep = ""))

spring <- spring %>%
  mutate(obs_value = spring_mean) %>%
  mutate(springyear = year) %>%
  select(cli_estacion_id, springyear, obs_value) %>%
  mutate(cli_estacion_id = as.factor(cli_estacion_id)) 

spring_ts <- inner_join(spring, timeseries)
```
...
```{r}
ggplot(spring_ts, aes(x = modelled_mean, y = obs_value, colour = codigo)) +
  geom_point() + 
  xlab("Predicted value") + ylab("Observed value") + 
  theme(legend.position="none") +
  ggtitle("Comparison of spring mean temperature predicted (mean across models) and observed values") +
  geom_abline(slope = 1, intercept = 0)
```

Modelled vs observations: one model
```{r}
ggplot(spring_ts, aes(x = tas_ipsl, y = obs_value, colour = codigo)) +
  geom_point() + 
  xlab("Predicted value") + ylab("Observed value") + 
  theme(legend.position="none") +
  ggtitle("Comparison of spring mean temperature predicted by model IPSL and observed") +
  geom_abline(slope = 1, intercept = 0)
```
...
ggplot(data = selection, aes(x = time_cnrm, y = value, colour = variable)) +
  geom_line() + 
  xlab("Year") + ylab("Spring mean temperature (ºC)") + 
  theme() +
...
```{r}
reshaped_ts <- melt(spring_ts, id = c("codigo", "springyear", "time_cnrm")) %>%
...
```{r}
a <- cor.test(spring_ts$obs_value, spring_ts$tas_cnrm)
b <- cor.test(spring_ts$obs_value, spring_ts$tas_ichec)
c <- cor.test(spring_ts$obs_value, spring_ts$tas_ipsl)
d <- cor.test(spring_ts$obs_value, spring_ts$tas_mohc)
e <- cor.test(spring_ts$obs_value, spring_ts$tas_mpi)
f <- cor.test(spring_ts$obs_value, spring_ts$modelled_mean)         
...
```{r}
meanofobservationdata <- mean(spring_ts$obs_value)

rmse <- spring_ts %>%
  select(codigo, springyear, obs_value, modelled_mean, modelled_sd) %>%
...
timeseries <- timeseries %>%
  mutate(time_cnrm = as.Date(format(time_cnrm, "%Y-%m-%d"))) %>%
  mutate(summeryear = as.integer(format(time_cnrm,"%Y"))) %>%
  select(codigo, cli_estacion_id, time_cnrm, summeryear, tas_cnrm, tas_ichec, tas_ipsl, tas_mohc, tas_mpi) %>%
  mutate(codigo = as.character(codigo)) %>%
  mutate(cli_estacion_id = as.factor(cli_estacion_id))

# Add field observations to time series dataframe
summer <- read.csv(paste(path, folder, "summer_mean.csv", sep = ""))

summer <- summer %>%
  mutate(obs_value = summer_mean) %>%
  mutate(summeryear = year) %>%
  select(cli_estacion_id, summeryear, obs_value) %>%
  mutate(cli_estacion_id = as.factor(cli_estacion_id)) 

summer_ts <- inner_join(summer, timeseries)
...
Modelled vs observations: mean of models
```{r}
ggplot(summer_ts, aes(x = modelled_mean, y = obs_value, colour = codigo)) +
  geom_point() + 
  xlab("Predicted value") + ylab("Observed value") + 
  theme(legend.position="none") +
  ggtitle("Comparison of summer mean temperature predicted (mean across models) and observed values") +
  geom_abline(slope = 1, intercept = 0)
```

Modelled vs observations: one model
```{r}
ggplot(summer_ts, aes(x = tas_ipsl, y = obs_value, colour = codigo)) +
  geom_point() + 
  xlab("Predicted value") + ylab("Observed value") + 
  theme(legend.position="none") +
  ggtitle("Comparison of summer mean temperature predicted by model IPSL and observed") +
  geom_abline(slope = 1, intercept = 0)
```
...
ggplot(data = selection, aes(x = time_cnrm, y = value, colour = variable)) +
  geom_line() + 
  xlab("Year") + ylab("Summer mean temperature (ºC)") + 
  theme() +
  ggtitle("Time series for station E190: observed and modelled") +
  scale_y_continuous(limits=c(0, 30))
```
...
Modelled vs observations: all-stations time series all models and mean
```{r}
reshaped_ts <- melt(summer_ts, id = c("codigo", "summeryear", "time_cnrm")) %>%
  filter(variable != "modelled_sd") %>%
  filter(variable != "cli_estacion_id") %>%
  mutate(codigo = as.factor(codigo)) %>%
  mutate(value = as.numeric(value))
...
```{r}
# Correlations between observations and predicted values: per model and against modelled mean
a <- cor.test(summer_ts$obs_value, summer_ts$tas_cnrm)
b <- cor.test(summer_ts$obs_value, summer_ts$tas_ichec)
c <- cor.test(summer_ts$obs_value, summer_ts$tas_ipsl)
d <- cor.test(summer_ts$obs_value, summer_ts$tas_mohc)
e <- cor.test(summer_ts$obs_value, summer_ts$tas_mpi)
f <- cor.test(summer_ts$obs_value, summer_ts$modelled_mean)         
...
meanofobservationdata <- mean(summer_ts$obs_value)

rmse <- summer_ts %>%
  select(codigo, summeryear, obs_value, modelled_mean, modelled_sd) %>%
  mutate(sq_error = (modelled_mean - obs_value) ^ 2) %>%
  mutate(deviationfromdata = (obs_value - meanofobservationdata) ^ 2)
...
timeseries <- timeseries %>%
  mutate(time_cnrm = as.Date(format(time_cnrm, "%Y-%m-%d"))) %>%
  mutate(fallyear = as.integer(format(time_cnrm,"%Y"))) %>%
  select(codigo, cli_estacion_id, time_cnrm, fallyear, tas_cnrm, tas_ichec, tas_ipsl, tas_mohc, tas_mpi) %>%
  mutate(codigo = as.character(codigo)) %>%
  mutate(cli_estacion_id = as.factor(cli_estacion_id))

# Add field observations to time series dataframe
fall <- read.csv(paste(path, folder, "fall_mean.csv", sep = ""))

fall <- fall %>%
  mutate(obs_value = fall_mean) %>%
  mutate(fallyear = year) %>%
  select(cli_estacion_id, fallyear, obs_value) %>%
  mutate(cli_estacion_id = as.factor(cli_estacion_id)) 

fall_ts <- inner_join(fall, timeseries)
...
Modelled vs observations: mean of models
```{r}
ggplot(fall_ts, aes(x = modelled_mean, y = obs_value, colour = codigo)) +
  geom_point() + 
  xlab("Predicted value") + ylab("Observed value") + 
  theme(legend.position="none") +
  ggtitle("Comparison of fall mean temperature predicted (mean across models) and observed values") +
  geom_abline(slope = 1, intercept = 0)
```

Modelled vs observations: one model
```{r}
ggplot(fall_ts, aes(x = tas_ipsl, y = obs_value, colour = codigo)) +
  geom_point() + 
  xlab("Predicted value") + ylab("Observed value") + 
  theme(legend.position="none") +
  ggtitle("Comparison of fall mean temperature predicted by model IPSL and observed") +
  geom_abline(slope = 1, intercept = 0)
```
...
ggplot(data = selection, aes(x = time_cnrm, y = value, colour = variable)) +
  geom_line() + 
  xlab("Year") + ylab("Fall mean temperature (ºC)") + 
  theme() +
  ggtitle("Time series for station E190: observed and modelled") +
  scale_y_continuous(limits=c(0, 30))
```
...
Modelled vs observations: all-stations time series all models and mean
```{r}
reshaped_ts <- melt(fall_ts, id = c("codigo", "fallyear", "time_cnrm")) %>%
  filter(variable != "modelled_sd") %>%
  filter(variable != "cli_estacion_id") %>%
  mutate(codigo = as.factor(codigo)) %>%
  mutate(value = as.numeric(value))
...
```{r}
# Correlations between observations and predicted values: per model and against modelled mean
a <- cor.test(fall_ts$obs_value, fall_ts$tas_cnrm)
b <- cor.test(fall_ts$obs_value, fall_ts$tas_ichec)
c <- cor.test(fall_ts$obs_value, fall_ts$tas_ipsl)
d <- cor.test(fall_ts$obs_value, fall_ts$tas_mohc)
e <- cor.test(fall_ts$obs_value, fall_ts$tas_mpi)
f <- cor.test(fall_ts$obs_value, fall_ts$modelled_mean)         
...
meanofobservationdata <- mean(fall_ts$obs_value)

rmse <- fall_ts %>%
  select(codigo, fallyear, obs_value, modelled_mean, modelled_sd) %>%
  mutate(sq_error = (modelled_mean - obs_value) ^ 2) %>%
  mutate(deviationfromdata = (obs_value - meanofobservationdata) ^ 2)
...
timeseries <- timeseries %>%
  mutate(time_cnrm = as.Date(format(time_cnrm, "%Y-%m-%d"))) %>%
  mutate(winteryear = as.integer(format(time_cnrm,"%Y"))) %>%
  select(codigo, cli_estacion_id, time_cnrm, winteryear, tas_cnrm, tas_ichec, tas_ipsl, tas_mohc, tas_mpi) %>%
  mutate(codigo = as.character(codigo)) %>%
  mutate(cli_estacion_id = as.factor(cli_estacion_id))

# Add field observations to time series dataframe
winter <- read.csv(paste(path, folder, "winter_mean.csv", sep = ""))

winter <- winter %>%
  mutate(obs_value = winter_mean) %>%
  mutate(winteryear = year_grouping) %>%
  select(cli_estacion_id, winteryear, obs_value) %>%
  mutate(cli_estacion_id = as.factor(cli_estacion_id)) 

winter_ts <- inner_join(winter, timeseries)
...
Modelled vs observations: mean of models
```{r}
ggplot(winter_ts, aes(x = modelled_mean, y = obs_value, colour = codigo)) +
  geom_point() + 
  xlab("Predicted value") + ylab("Observed value") + 
  theme(legend.position="none") +
  ggtitle("Comparison of winter mean temperature predicted (mean across models) and observed values") +
  geom_abline(slope = 1, intercept = 0)
```

Modelled vs observations: one model
```{r}
ggplot(winter_ts, aes(x = tas_ipsl, y = obs_value, colour = codigo)) +
  geom_point() + 
  xlab("Predicted value") + ylab("Observed value") + 
  theme(legend.position="none") +
  ggtitle("Comparison of winter mean temperature predicted by model IPSL and observed") +
  geom_abline(slope = 1, intercept = 0)
```
...
ggplot(data = selection, aes(x = time_cnrm, y = value, colour = variable)) +
  geom_line() + 
  xlab("Year") + ylab("Winter mean temperature (ºC)") + 
  theme() +
  ggtitle("Time series for station E190: observed and modelled") +
  scale_y_continuous(limits=c(0, 30))
```
...
Modelled vs observations: all-stations time series all models and mean
```{r}
reshaped_ts <- melt(winter_ts, id = c("codigo", "winteryear", "time_cnrm")) %>%
  filter(variable != "modelled_sd") %>%
  filter(variable != "cli_estacion_id") %>%
  mutate(codigo = as.factor(codigo)) %>%
  mutate(value = as.numeric(value))
...
```{r}
# Correlations between observations and predicted values: per model and against modelled mean
a <- cor.test(winter_ts$obs_value, winter_ts$tas_cnrm)
b <- cor.test(winter_ts$obs_value, winter_ts$tas_ichec)
c <- cor.test(winter_ts$obs_value, winter_ts$tas_ipsl)
d <- cor.test(winter_ts$obs_value, winter_ts$tas_mohc)
e <- cor.test(winter_ts$obs_value, winter_ts$tas_mpi)
f <- cor.test(winter_ts$obs_value, winter_ts$modelled_mean)         
...
meanofobservationdata <- mean(winter_ts$obs_value)

rmse <- winter_ts %>%
  select(codigo, winteryear, obs_value, modelled_mean, modelled_sd) %>%
  mutate(sq_error = (modelled_mean - obs_value) ^ 2) %>%
  mutate(deviationfromdata = (obs_value - meanofobservationdata) ^ 2)
